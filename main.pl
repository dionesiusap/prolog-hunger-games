setup :- consult(enemy),consult(map),consult(player),consult(item),consult(terrain),consult(attack),consult(save),consult(commands),initPlayer,initTerrain,initItem,initMap,initEnemy.
