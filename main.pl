setup :- consult(enemy),consult(save),consult(map),consult(player),consult(item),consult(terrain),consult(attack),consult(commands),initPlayer,initTerrain,initItem,initItemDrop,initMap,initEnemy.
