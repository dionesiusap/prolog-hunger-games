...
sleep:-player(X,Y,S,Lp,H,T,W,L),lookTerrainNow(X,Y,3),!, retract(player(X,Y,S,Lp,H,T,W,L)), asserta(player(X,Y,S,F,H,100,W,L)),write('You found an empty camp near the lake. You tried to sleep there.\nYou had unlimited supply of water for the night and now you\'re fully hydrated.\n'),moveenemy,moveenemy,moveenemy,moveenemy,moveenemy.
